...
class TestStorageIODevices(TestBase):
    ...
    def test_S001_retrieve_all_storagedevices(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S001_retrieve_all_storagedevices()')
        try:
            self.logging.debug(
                'Retrieving summarized list of all storage i/o devices of '
                'type %s and %s using URI %s' % (DASD_ECKD, ZFCP, self.uri_storagedevices))
            resp = self.session.request_get_json(self.uri_storagedevices, expected_status_values=[200])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                for each_json in resp:
                    self.logging.debug('validating json %s against default json schema of storage i/o device' % each_json)
    ...
    def test_S002_retrieve_dasdeckd_storagedevices(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S002_retrieve_dasdeckd_storagedevices()')
        try:
            uri_dasdeckd = self.uri_storagedevices + '?_type=' + DASD_ECKD
            self.logging.debug(
                'Retrieving summarized list of all storage i/o devices of'
                ' type %s using URI %s' % (DASD_ECKD, uri_dasdeckd))
            resp = self.session.request_get_json(uri_dasdeckd, expected_status_values=[200])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                for each_json in resp:
                    self.logging.debug(
    ...
    def test_S003_retrieve_zfcp_storagedevices(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S003_retrieve_zfcp_storagedevices()')
        try:
            uri_zfcp = self.uri_storagedevices + '?_type=' + ZFCP
            self.logging.debug(
                'Retrieving summarized list of all storage i/o devices of'
                ' type %s using URI %s' % (ZFCP, uri_zfcp))
            resp = self.session.request_get_json(uri_zfcp, expected_status_values=[200])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                for each_json in resp:
                    self.logging.debug('validating json %s against default json schema of storage i/o device' % each_json)
    ...
    def test_F004_retrieve_storagedevices_invalidtype(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_F004_retrieve_storagedevices_invalidtype()')
        try:
            ...
            resp = self.session.request_get_json(uri_invalid_type, expected_status_values=[400])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                self.logging.info('As expected, received 400 status code')
            else:
                self.logging.debug('Received None/Empty json response instead of 400 status code')
                self.fail(
    ...
    def test_S005_get_storagedevice(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S005_get_storagedevice()')
        self.logging.debug(
            'Retrieving summarized list of all storage i/o devices of '
            'type %s and %s using URI %s' % (DASD_ECKD, ZFCP, self.uri_storagedevices))
        resp = self.session.request_get_json(self.uri_storagedevices, expected_status_values=[200])
        if resp:
            ...
            for each_json in resp:
                self.logging.debug(
                    'validating json %s against default json schema of storage i/o device' % each_json)
                self.validator.validate_json(
                    each_json, self.storagedevices_schema)
            ...
            try:
                self.logging.debug('Retrieving detailed information of '
                                   'storage i/o device %s using uri'
                                   ' %s' % (storage_device, device_uri))
                ...
                if device_resp:
                    ...
                    self.validator.validate_json(device_resp, self.storagedevices_schema)
                else:
                    self.logging.error('Failed to fetch details of storage'
                                       ' i/o device %s. Received None/Empty'
                                       ' response' % storage_device)
                    self.fail('Failed to fetch details of storage i/o '
    ...
    def test_S006_get_dasdeckd_device(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S006_get_dasdeckd_device()')
        ...
        try:
            self.logging.debug(
                'Retrieving detailed information of a dasd-eckd storage i/o device %s '
                'using URI %s' % (dasdeckd_device, device_uri))
            resp = self.session.request_get_json(device_uri, expected_status_values=[200])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                self.logging.debug(
                    'validating json %s against default json schema of storage i/o device' % resp)
                self.validator.validate_json(
                    resp, self.storagedevices_schema)
            else:
                self.logging.error('Failed to fetch details of dasd-eckd '
                                   'storage i/o device %s. Received None/Empty'
                                   ' response' % dasdeckd_device)
                self.fail('Failed to fetch details of dasd-eckd storage i/o '
                              'device %s. Received None/Empty '
                              'response' % dasdeckd_device)
        except APIRequestError as error:
            self.logging.error(error.__str__())
    ...
    def test_S007_get_zfcp_device(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S007_get_zfcp_device()')
        ...
        try:
            self.logging.debug(
                'Retrieving detailed information of a zfcp storage i/o device %s '
                'using URI %s' % (zfcp_device, device_uri))
            resp = self.session.request_get_json(device_uri, expected_status_values=[200])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                self.logging.debug(
                ...
                self.logging.error('Failed to fetch details of zfcp '
                                   'storage i/o device %s. Received None/Empty'
                                   ' response' % zfcp_device)
                self.fail('Failed to fetch details of zfcp storage i/o '
                              'device %s. Received None/Empty '
                              'response' % zfcp_device)
        except APIRequestError as error:
            self.logging.error(error.__str__())
    ...
    def test_F008_get_invalid_device(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_F008_get_invalid_device()')
        device_id = 'invalid_device'
        device_uri = self.uri_storagedevices + '/' + device_id
        try:
            self.logging.debug(
                'Retrieving detailed information of an invalid storage i/o device %s '
    ...
    def test_S009_dasdeckd_online_valid(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S009_dasdeckd_online_valid()')
        ...
        device_uri = self.uri_storagedevices + '/' +\
        ...
        try:
            ...
            resp = self.session.request_post_json(device_uri, expected_status_values=[200])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                self.logging.debug('validating json %s against default json'
                                   ' schema of storage i/o device' % resp)
    ...
    def test_S010_dasdeckd_offline_valid(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S010_dasdeckd_offline_valid()')
    ...
    def test_S011_zfcp_online_valid(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S011_zfcp_online_valid()')
    ...
    def test_S012_zfcp_offline_valid(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_S012_zfcp_offline_valid()')
    ...
    def test_F013_invalid_online(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_F013_invalid_online()')
        ...
        try:
            ...
            resp = self.session.request_post_json(device_uri, expected_status_values=[400])
            if resp:
                self.logging.debug('Response received: %s' % resp)
                self.logging.debug(
    ...
    def test_F014_invalid_offline(self):
        ...
        """
        self.logging.info('-->TestStorageIODevices.test_F014_invalid_offline()')
        invalid_device = 'invalid_device'
        device_uri = self.uri_storagedevices + '/' +\
